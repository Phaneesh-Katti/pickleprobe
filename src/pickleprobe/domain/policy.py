...
def load_policy(path: Path | None = None) -> SecurityPolicy:
    ...
    if path is None:
        raw = resources.files("pickleprobe.policy").joinpath("default.yaml").read_text(encoding="utf-8")
    else:
...
_default_policy_path: str | None = None


def configure_policy(path: str | Path | None = None) -> None:
    """Set the default policy file for subsequent analysis (clears cache)."""
    global _default_policy_path
    _default_policy_path = str(path) if path is not None else None
    get_policy.cache_clear()


@lru_cache(maxsize=8)
def get_policy(path: str | None = None) -> SecurityPolicy:
    effective = path if path is not None else _default_policy_path
    return load_policy(Path(effective) if effective else None)
